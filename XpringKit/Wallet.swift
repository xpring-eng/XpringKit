...
public class Wallet {
	...
	public var address: Address { return walletJS.address }

	/// Returns a hex encoded public key corresponding to this `Wallet`.
	public var publicKey: Hex { return walletJS.publicKey }

	/// Returns a hex encoded private key corresponding to this `Wallet`.
	public var privateKey: Hex { return walletJS.privateKey }
	...
	public convenience init?(mnemonic: String, derivationPath: String? = nil) {
		guard let walletJS = Wallet.javaScriptWalletFactory.wallet(
			mnemonic: mnemonic,
	...
	public convenience init?(seed: String) {
		guard let walletJS = Wallet.javaScriptWalletFactory.wallet(seed: seed) else {
			return nil
		}
		self.init(walletJS: walletJS)
	}
