...
public enum ILPError: Error {
  ...
  /// Translate an Error thrown from an ILPNetworkClient to an ILPError.
  ///
  /// gRPC services return an error with a status code so we need to map gRPC error status to native ILPErrors.
  ///
  /// - Parameters:
