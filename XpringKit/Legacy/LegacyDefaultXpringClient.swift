...
extension LegacyDefaultXpringClient: XpringClientDecorator {
    public func getRawTx(for transactionHash: TransactionHash) throws -> Rpc_V1_GetTxResponse {
        ...
        let resp = try networkClient.getTransactionStatus(transactionStatusRequest)
        return Rpc_V1_GetTxResponse.with {
            $0.ledgerIndex = resp.lastLedgerSequence
            $0.meta.transactionResult.result = resp.transactionStatusCode
            $0.validated = resp.validated
        }
    }
