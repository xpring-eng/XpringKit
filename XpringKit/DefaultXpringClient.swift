...
extension DefaultXpringClient: XpringClientDecorator {
  ...
  public func getTransactionHistory(for address: Address) throws -> [XRPTransaction] {
    ...
    let getTransactionResponses = transactionHistory.transactions

    return try getTransactionResponses.map { getTransactionResponse in
      let transaction = getTransactionResponse.transaction
      guard let xrpTransaction = XRPTransaction(transaction: transaction) else {
        throw XRPLedgerError.malformedResponse("Unable to parse transaction: \(transaction)")
      }
