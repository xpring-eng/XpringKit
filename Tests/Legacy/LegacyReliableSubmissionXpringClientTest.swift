...
final class LegacyReliableSubmissionClientTest: XCTestCase {
  ...
  func testSendWithExpiredLedgerSequenceAndUnvalidatedTransaction() throws {
    ...
    do {
      _ = try reliableSubmissionClient.send(BigUInt(10), to: .testAddress, from: .legacyTestWallet)
      expectation.fulfill()
    } catch {
  ...
  func testSendWithNoLastLedgerSequence() throws {
    ...
    XCTAssertThrowsError(try reliableSubmissionClient.send(BigUInt(10), to: .testAddress, from: .legacyTestWallet))
  }
